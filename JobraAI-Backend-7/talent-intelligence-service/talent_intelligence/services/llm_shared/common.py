...
OPENAI_TEMPERATURE = max(0.0, min(1.0, float(os.getenv("OPENAI_TEMPERATURE", "0.1"))))
...
MIN_LLM_RESPONSE_SECONDS = max(0.0, float(os.getenv("MIN_LLM_RESPONSE_SECONDS", "10")))
MAX_LLM_RESPONSE_SECONDS = max(MIN_LLM_RESPONSE_SECONDS, float(os.getenv("MAX_LLM_RESPONSE_SECONDS", "15")))
...
def _chat_completions_create_with_retry(
    *,
    client: OpenAI,
    model: str | object,
    messages: list[dict],
    ...
    raise ValueError(f"OpenAI chat completion failed: {last_exc}") from last_exc
...
def enforce_min_llm_latency(start_time: float) -> None:
    """Keep the UX steady when LLM mode is active by holding responses to a configured minimum window."""
    target = random.uniform(MIN_LLM_RESPONSE_SECONDS, MAX_LLM_RESPONSE_SECONDS)
...
def _normalize_full_analysis_payload(parsed: object) -> dict:
    ...
    out["Score_Breakdown"] = {
        ...
        "EDUCATION": _int_0_100(sb.get("EDUCATION"), 0),
    }
...
def _normalize_general_ats_payload(parsed: object) -> dict:
    ...
    if not isinstance(sb, dict):
        sb = {}
    out["Score_Breakdown"] = {
        "FORMAT": f"{_int_0_100(sb.get('FORMAT'), 0)}%",
        "SKILLS": f"{_int_0_100(sb.get('SKILLS'), 0)}%",
        "EXPERIENCE": f"{_int_0_100(sb.get('EXPERIENCE'), 0)}%",
        "EDUCATION": f"{_int_0_100(sb.get('EDUCATION'), 0)}%",
    }
...
def llm_policy() -> dict[str, str]:
    key_loaded = openai_key_available()
    return {
